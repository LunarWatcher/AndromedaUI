...
#include <andromeda/dom/direction.hpp>  // for Direction, Direction::Left, Direction::Right, Direction::Down
#include <andromeda/dom/elements.hpp>  // for Element, separator
#include <andromeda/util/ref.hpp>      // for Ref, ConstRef, StringRef
...
struct ResizableSplitOption {
  ...
  Ref<int> main_size =
      (direction() == Direction::Left || direction() == Direction::Right) ? 20
                                                                          : 10;
  std::function<Element()> separator_func = [] { return ::andromeda::separator(); };
};
