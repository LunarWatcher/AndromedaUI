...
#include "andromeda/screen/deprecated.hpp"       // for wchar_width, wstring_width
#include "andromeda/screen/string_internal.hpp"  // for WordBreakProperty, EatCodePoint, CodepointToWordBreakProperty, GlyphCount, GlyphIterate, GlyphNext, GlyphPrevious, IsCombining, IsControl, IsFullWidth, Utf8ToWordBreakProperty
...
using WBP = andromeda::WordBreakProperty;
...
int codepoint_width(uint32_t ucs) {
  if (andromeda::IsControl(ucs)) {
    return -1;
  }

  if (andromeda::IsCombining(ucs)) {
    return 0;
  }

  if (andromeda::IsFullWidth(ucs)) {
    return 2;
  }
